...
function addAPost() {
    ...
    newPostTitle.innerText = `Post ${numberOfPosts}`;
    numberOfPosts += 1;
    newPost.appendChild(newPostTitle);
    let newPostBody = document.createElement('p');
    let randomIndex = Math.floor(Math.random() * possibleBodys.length);
    console.log(`Adding post body ${randomIndex + 1}`);
    newPostBody.innerText = possibleBodys[randomIndex];
